/// ADR-0057: the miner row, the store-less Work body that stands over the
/// mineral container and digs the season's Thorium — its sizing rule, the
/// quota that opens and closes it, the cut that tells it from the anchor,
/// and its place in the casting cascade.
...
/// source walled in at either end and a built container on each one's only
/// Seat — two posts, so both rocks are income the surplus counts — at a 550
/// bank. `mineColony` carries no source at all on purpose, so this is the
/// one colony in the suite whose surplus has a number for the miner's
/// charge to come out of (#304).
///
/// Neither source container may stand on a seat of the deposit, which is
/// why the west source is at x = 4 rather than at the corridor's old mouth:
/// `Atlas.postsOf` reads a mine post as (any mineral's Seats) ∩ (any
/// container tile in the room), so an energy container on (9,10), `min-a`'s
/// other Seat, is a second mine Post holding no Thorium. The gate itself is
/// #261's and is filed as #312; what is fixed here is the fixture.
    ...
    testList
        ...
        [
            test "the miner row buys Work up to twenty, with one Move per five" {
                // The two banks the colony will actually stand at: RCL5's 1,800 and
                // RCL6's 2,300. The Move is bought for a single commute and not for
                // fatigue parity — the body walks to one tile once and never leaves it
                // — so the ratio is the row's own and the remainder is spent on Work.
                Expect.equal
            ...
            test "the Work each Move carries is the colony's own number" {
                // `Tuning.MinerWorkPerMove` pairwise, one field apart and at the bank
                // the ratio was derived at: it is a knob a human turns in a commit, so
                // the sizing rule has to read the colony's own and not a constant.
                let bodyAt perMove =
                    sizedBodyFor
                        { largestSizing with
    ...
    testList
        ...
        [
            ...
            test "a deposit the mod has deleted hires nobody" {
                // The stop condition: the mod removes an exhausted Thorium deposit
                // outright, so the target leaves the projection and the row reads 0 off
                // the same fact that gave it 1.
                Expect.equal
            ...
            test "the miner row is an addend of the Workforce target" {
                // A miner left out of the target would have the deficit read the body
                // as one of the generalists the income already paid for, quietly
                // retiring a worker for its whole life. Read under a Workforce floor of
                // one, which is the premise: `Tuning.MinWorkforce` is two, and a colony
                // this small stands on the floor either side of the pair, where a
                // difference of one addend is invisible. `mineColony` has no source at
                // all, so its income is zero and the surplus the miner is charged
                // against (#304, below) is at or under zero either side: what the
                // addend costs the generalist row is the case beneath this one.
                let targetOf colony =
                    (decideOn
                        { colony with
            ...
            test "the miner row's replacement is deducted from the surplus, three bodies of it" {
                // #304: the row is an addend of the target and a term of the
                // amortization, the reserver, anchor and hauler rows' own shape. A
                // miner charged nowhere is an upgrade mouth's worth of income the
                // colony sells twice for as long as a deposit lasts.
                //
                // Three bodies over one worker's life, scaled as the reserver's
                // 600-tick body is scaled onto the 1,500 this sum is written in: the
                // miner stands on the mineral container, `thorium.js` takes
                // `floor(log10 store.T)` off the `ageTime` of everything on that tile
                // every tick, and a container the haul keeps inside the 100..999 band
                // therefore burns `Tuning.MineContactAgeing` (3) ticks of the body's
                // life a tick.
                //
                // Read from both sides, because one worker unit is about six miner
                // bodies wide and a single income pins a band and not a number: at the
                // 550 bank a charge of two bodies still hires the eighth generalist,
                // and at the 500 bank a charge of four retires one the third does not.
                // Only one rounding boundary ever falls inside the range at one bank,
                // which is why this case is two colonies.
                ...
                // The 500 bank, from above: the same colony one cast smaller, where
                // the rounding boundary sits between the third body and the fourth.
                // This is also `MineContactAgeing`'s own pairwise, and the reading that
                // says the charge is three and not merely "at least three".
    ...
    testList
        ...
        [
            test "a miner fills the miner row's Living and no other row's" {
                // The row is read back off the parts, and Work-heavy-with-no-Carry is
                // the one cut no other row of this colony makes. Without the arm a
                // `[20 Work; 4 Move]` is Work-heavy and falls through to the anchor
                // row, filling a quota counted off the posts and retiring a garrison
                // from a rock for 1,500 ticks. Pairwise, one body apart.
                let livingOf colony =
            ...
            test "the row casts the body its bank affords, named for the row" {
                // The supply floor stands in front of every row, so the colony is
                // given a carrier that pays it off before the miner's own seat is
                // reached.
                let colony =
                    { richMine 1800 with
    ...
    testList
        ...
        [
            ...
            test "a light body is offered no tile of a deposit and no slot on it" {
                // #261: a `[16 Work; 4 Move]` miner loses Work parts head-first to
                // damage, and at `[3 Work; 4 Move]` it is no longer Work-heavy but
                // still answers the Emitter's mineral arm, a Work part and no Carry.
                // The light-body complement rule would then hand it the deposit's
                // Seats less the mine Post, steering it off the container onto bare
                // ground, where a store-less dig drops the Thorium and the pile bleeds
                // `ceil(amount / 1000)` a tick with nothing to pick it up. A deposit
                // has no half the garrison is not draining, so it has no light body's
                // tile and no light body's slot.
    ...
    testList
        ...
        [
            test "the mineral container is one more line in the hauler row's reported demand" {
                ...
                // rate — `Work / 6` Thorium a tick, the extractor's cooldown being five
                // and the intent pass running before the object pass — the
                // `output × trip` shape every source container's line already has.
                // Pairwise on the container alone: with the mine container not yet
                // standing there is no mine and nothing asked for.
            ...
            test "a mine the colony cannot bank to asks for no haul at all" {
                ...
                // the Refill needing a Storage with room. Pairwise against the case
                // above, one structure apart.
                Expect.isEmpty
            ...
            test "a mine that cannot be dug asks for no haul either" {
                // #262: the term prices the miner's output, so it must read the same
                // facts the miner row's own quota puts a body at 0 for. The container
                // alone bought a carrier for a mine producing nothing, and both halves
                // are ordinary: the Layout emits the extractor and the container as
                // two 5,000-point sites in one tick and which finishes first is the
                // builder's accident, the RCL6 build window this leg lands in.
