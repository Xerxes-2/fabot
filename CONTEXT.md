...
### Arrival
The tick a creep can first act on a Task: its [[travel cost]] in ticks from where it stands now. The horizon at which time-aware judgements are made, not the current tick (ADR 0025, ADR 0026): a drained source's Harvest is applicable when the walk covers the [[restock]] wait; a creep is [[expiring]] when its successor's [[lead]] outlasts its life; a Task's capacity counts only holders still alive when the candidate arrives. Derived each tick from the Snapshot and the [[atlas]], never persisted.
...
### Lead
The time a creep's replacement needs to stand on its tile: the successor body's cast time plus that body's [[travel cost]] from the spawn, priced for the successor's fatigue factor, not the incumbent's (ADR 0026). One rule for every [[body pattern]] — a slow Anchor earns a long lead, a hauler on a [[trunk]] a short one — never a per-row constant.
