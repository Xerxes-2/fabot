...
### Rampart
The walkable defensive structure the [[layout]] places over every [[keep]] structure and every [[post]] the tick the thing it covers stands (ADR 0034). A creep on its own rampart cannot be attacked, so the tile is in no [[reach]] — the [[anchor]]'s whole defence, since it cannot [[flee]]. Kept above a **floor** of hits by [[repair]]: hungry below it, whole at it, the number a tunable derived from the ticks to hold times the damage per tick to hold against, never a term of the decision. Placed at any level; the set is the rule's, so a three-source room ramparts three Posts.
