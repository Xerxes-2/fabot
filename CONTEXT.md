...
### Link footing
A tile the [[layout]] reserves for a link (ADR 0022) while the level that unlocks links is still ahead: one per [[post]], one beside the controller [[container]], one beside the [[storage]] — each the buildable tile within range 1 of its target, off every [[trunk]], nearest the spawn. The only structure footing allowed on [[working ground]]: a link there buys the [[anchor]] and the upgraders a transfer without leaving their tile. Which footings are filled first, and what the links do, is RCL5's decision — the footing exists so an extension never claims the tile in the meantime.
