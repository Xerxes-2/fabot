...
### Lead
The time a creep's replacement needs to stand on its tile: the successor body's cast time (3 ticks a part) plus that body's walk out of the spawn, priced over [[travel cost]]'s own weights for the successor's fatigue factor, not the incumbent's (ADR 0026) — but blind to today's traffic, as the hauler quota's round trip is: the walk does not start until the body is cast, and the tile it ends on is the one the creep being replaced is standing on. The walk begins *beside* the spawner, where the engine places a finished creep, never on the spawner's own tile — a step the replacement never walks would buy a lead it cannot use, and the traffic-blind lead against the traffic-priced [[arrival]] already leaves the succession exactly one [[occupancy surcharge]] of slack. One rule for every [[body pattern]] — a slow Anchor earns a long lead, a hauler on a [[trunk]] a short one — never a per-row constant. Several spawns resolve as the quota resolves them, at the cheapest; geometry the [[atlas]] cannot price leads nobody.
