...
| what the colony says and does off the pipeline — the [[signature reflex]] and the other colony reflexes | `SignatureTests.fs` |
| the errand — a room a colony declares for one named object in it, and the work that declaration buys: the `Reclaim` Task, who may hold it and the act it fires, plus the courier's paired Storage→Reactor delivery whose far endpoint only that declaration supplies (ADR 0060 decision 3, ADR 0067; the declaration kind itself is decision 1's). The errand's *projection* half — what a colony may carry of that room at all — is `ViewTests`', not this | `ErrandTests.fs` |
