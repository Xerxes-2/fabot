...
| the errand — a room a colony declares for one named object in it, and the work that declaration buys: the `Reclaim` Task, who may hold it and the act it fires (ADR 0060 decision 3; the declaration kind itself is decision 1's). The errand's *projection* half — what a colony may carry of that room at all — is `ViewTests`', not this | `ErrandTests.fs` |
