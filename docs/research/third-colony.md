...
> **本节第一条已由 `fourth-colony.md`（2026-09-16）接手并改正一条事实**：W11S29 确实成了第四个房的选择，但本节写的“非 highway 邻居只有 W11S28 与 W12S29”是错的 —— 按 ADR 0062 的口径重数，`W11S28 ↔ W11S29` 是 **0 格**，所以它只有 W12S29 一个可声明的邻居；本节说的“到 Reactor 8 hop”成立，而那条路今天谁也走不了，交付要靠 terminal。同一份文档还实测了本节没算的 W9S28（同样 d4 45,000、两个 source）**完全不可达**。
