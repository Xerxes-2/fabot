...
Guard emits a single attack-or-heal candidate. Pickup production now emits one target per creep: an emitted task pickup owns the channel; otherwise the reflex keeps the last reachable target in Atlas order, preserving the target the engine previously retained after overwriting earlier calls. Neither choice is made by the validator.
