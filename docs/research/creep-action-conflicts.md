...
This convenient single group is specific to the current action vocabulary. `rangedHeal` joins it without breaking it (#409): the table orders `heal > rangedHeal > repair > build > attack > harvest` totally. `rangedAttack` does break it (#411), and `IntentPlan` now gives an intent a set of channels: ranged heal, repair and build take `Exclusive` and `Ranged`, and a ranged attack takes `Ranged` alone. The full engine table is not a disjoint partition once the ranged attacks arrive: `rangedHeal` suppresses `attack` and `rangedAttack`, but `attack` and `rangedAttack` can coexist; `heal` suppresses `rangedHeal` and `attack`, but does not suppress `rangedAttack`. That is why the representation was revisited rather than every combat action put into one slot. [Source: full priorities table](https://github.com/screeps/engine/blob/80977824199a596d174d392fd0cf8c458c21fcbd/src/processor/intents/creeps/intents.js#L3-L13).
