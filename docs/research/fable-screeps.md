...
- The MMO runtime runs each player's code in a persistent **isolated-vm** isolate (the driver source says **256 MB heap** + terrain data; `Game.cpu.getHeapStatistics().heap_size_limit` read **729–764 MB** on this account, 2026-09-21) that survives between ticks; global resets recompile everything, so bundle size and startup cost matter. CPU: 20 ms/tick baseline (more with subscription/GCL), bucket up to 10,000, max 500 CPU per tick from bucket.
