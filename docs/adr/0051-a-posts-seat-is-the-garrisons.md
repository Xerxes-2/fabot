...
> **Status:** amended by #216 R5

> **Amended by #216 R5** on where the light cap is written: `seats − posts` is now the share for every class **but** `Heavy` (`Capacity.Commuters`) of the pooled Harvest's `Capacity`, beside ADR 0024's `Garrisons` share and the Seat count as the total — the scope and not a class, which is the exact preservation of the `not workHeavy` reading this cap always had — three numbers that sum back to the Seat count exactly, set by the Planner (ADR 0052 decision 6). The Work Area narrowing and the cap's reading are unchanged.
