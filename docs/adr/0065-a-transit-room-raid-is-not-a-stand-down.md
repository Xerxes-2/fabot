...
> **Read beside #382** (ADR 0074), which leaves this decision standing and records a question about it: the rule below is about a **hostile** opening a raid stand-down, and W15S26's row exists because a **core** opens one on a different basis, in a room that is transit-only for this colony. Whether that is the intended reading or a gap here is undecided; ADR 0074 reads the row however it came to be.
...
We decided **a hostile opens a raid stand-down only in a room the colony can act on: a declared [[outpost]], or an [[errand]] target under ADR 0075's separate rule**. A hostile in a transit-only room opens no stand-down there, whether it is a Source Keeper, an Invader or another player. The boundary is the room's role, not the hostile's owner.
